...
export async function POST(req) {
    ...
    try {
        ...
        // Optionally, you can update the user’s role to 'vet' in the users table if needed
        await client.query('UPDATE users SET role = $1 WHERE user_id = $2', ['vet', user_id]);

        return new Response(JSON.stringify(vetResult.rows[0]), {
            status: 201,
            headers: { 'Content-Type': 'application/json' },
        });
    } catch (err) {
...
export async function PUT(req) {
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return new Response(JSON.stringify({ error: 'Vet not found' }), {
                status: 404,
                headers: { 'Content-Type': 'application/json' },
            });
        }
...
export async function DELETE(req) {
    ...
    try {
        ...
        return new Response(JSON.stringify({ message: 'Vet deleted successfully' }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        });
    } catch (err) {
