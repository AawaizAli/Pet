...
export async function GET(_: NextRequest) {
  ...
  try {
    ...
    const query = `
      ...
      INNER JOIN 
        vet_verification_application ON vet_qualifications.qualification_id = vet_verification_application.qualification_id
      WHERE 
    ...
    return NextResponse.json(
      { error: "Failed to fetch vets", details: (error as Error).message },
      { status: 500 }
    );
  } finally {
    try {
