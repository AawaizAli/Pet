...
export default function FosterPets() {
  ...
  const [filters, setFilters] = useState({
    isAdopt: false, // Foster by default
    isBuy: false,
    selectedSex: "",
    minAge: "",
    maxAge: "",
  ...
  // Fetch pets from the foster pets API
  useEffect(() => {
    const fetchFosterPets = async () => {
      try {
        setLoading(true); // Set loading state
        const response = await fetch("/api/foster-pets-objects");
        if (!response.ok) throw new Error("Failed to fetch foster pets");

        const data = await response.json();
        setPets(data);
      } catch (err) {
        console.log(err);
        setError("Failed to load foster pets.");
      } finally {
  ...
  const filteredPets = pets.filter((pet) => {
    const matchesType = !filters.isAdopt && pet.listing_type === "foster"; // Foster listing type
